<template>
    ...
    <OverallEmploymentDataReportModal ref="overallEmploymentDataReportModal" :pdf-url="pdfUrl" />
</template>
...
import { reportEmploymentDataService } from '@/services/report-employment-data.service';
import OverallEmploymentDataReportModal from '@/components/modal/reports/overall-employment-data-report-modal.vue';

export default {
    components: {
        ReportRow,
        OverallEmploymentDataReportModal
    },
    data() {
        return {
            report: {
    ...
    methods: {
        async preview() {
            try {
                ...
                const pdfBlob = await reportEmploymentDataService.generateEmploymentDataReportPDF(startDate, endDate);

                // Create a URL for the blob
                this.pdfUrl = window.URL.createObjectURL(pdfBlob);

                // Open the modal using the reference
                this.$refs.overallEmploymentDataReportModal.openModal();
        ...
        async exportPDF() {
            try {
                ...
                a.href = url;
                a.download = `employment-data-report-${startDate}-to-${endDate}.pdf`;
        ...
        async exportExcel() {
            try {
                ...
                const excelBlob = await reportEmploymentDataService.generateEmploymentDataReportExcel(startDate, endDate);

                // Create a URL for the blob and trigger download
                const url = window.URL.createObjectURL(excelBlob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `employment-data-report-${startDate}-to-${endDate}.xlsx`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
        ...
        exportCSV() {
            message.success('Exported Employment Data Report as CSV');
        }
    },
    mounted() {

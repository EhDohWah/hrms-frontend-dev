<template>
    ...
    <OverallTravelReportModal ref="overallTravelReportModal" :pdf-url="pdfUrl" />
</template>
...
import { reportTravelService } from '@/services/report-travel.service';
import OverallTravelReportModal from '@/components/modal/reports/overall-travel-report-modal.vue';

export default {
    components: {
        ReportRow,
        OverallTravelReportModal
    },
    data() {
        return {
            report: {
    ...
    methods: {
        async preview() {
            try {
                ...
                const pdfBlob = await reportTravelService.generateTravelReportPDF(startDate, endDate);

                // Create a URL for the blob
                this.pdfUrl = window.URL.createObjectURL(pdfBlob);

                // Open the modal using the reference
                this.$refs.overallTravelReportModal.openModal();
        ...
        async exportPDF() {
            try {
                ...
                a.href = url;
                a.download = `travel-report-${startDate}-to-${endDate}.pdf`;
        ...
        async exportExcel() {
            try {
                ...
                this.$message.loading({ content: 'Generating Excel...', key: 'excelExport' });

                // Use the service to generate the Excel
                const excelBlob = await reportTravelService.generateTravelReportExcel(startDate, endDate);

                // Create a URL for the blob and trigger download
                const url = window.URL.createObjectURL(excelBlob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `travel-report-${startDate}-to-${endDate}.xlsx`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
        ...
        async exportCSV() {
            try {
                ...
                this.$message.loading({ content: 'Generating CSV...', key: 'csvExport' });

                // Use the service to generate the CSV
                const csvBlob = await reportTravelService.generateTravelReportCSV(startDate, endDate);

                // Create a URL for the blob and trigger download
                const url = window.URL.createObjectURL(csvBlob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `travel-report-${startDate}-to-${endDate}.csv`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);

                message.success({ content: 'CSV generated successfully!', key: 'csvExport' });
            } catch (error) {
                console.error('Error exporting CSV:', error);
                message.error({ content: 'Failed to generate CSV. Please try again.', key: 'csvExport' });
            }
        }
    },
    mounted() {

<template>
    ...
    <OverallResignationReportModal ref="overallResignationReportModal" :pdf-url="pdfUrl" />
</template>
...
import { reportResignationService } from '@/services/report-resignation.service';
import OverallResignationReportModal from '@/components/modal/reports/overall-resignation-report-modal.vue';

export default {
    name: 'ResignationReport',
    components: {
        ReportRow,
        OverallResignationReportModal
    },
    data() {
        return {
            report: {
                ...
                description: "View all resignation reports",
                path: "/administration/reports/resignation-report",
                icon: "user-off"
            },
            pdfUrl: null
        };
    ...
    methods: {
        async preview() {
            try {
                ...
                const pdfBlob = await reportResignationService.generateResignationReportPDF(startDate, endDate);

                // Create a URL for the blob
                this.pdfUrl = window.URL.createObjectURL(pdfBlob);

                // Open the modal using the reference
                this.$refs.overallResignationReportModal.openModal();
        ...
        async exportPDF() {
            try {
                ...
                a.href = url;
                a.download = `resignation-report-${startDate}-to-${endDate}.pdf`;
        ...
        async exportExcel() {
            try {
                ...
                const excelBlob = await reportResignationService.generateResignationReportExcel(startDate, endDate);

                // Create a URL for the blob and trigger download
                const url = window.URL.createObjectURL(excelBlob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `resignation-report-${startDate}-to-${endDate}.xlsx`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
        ...
        exportCSV() {
            message.success('Exported Resignation Report as CSV');
        }
    }
};
</script>

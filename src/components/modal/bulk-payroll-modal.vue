          ...
          <div class="step-content">
            ...
            <div v-show="currentStep === 1">
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <a-date-picker v-model:value="formData.payPeriodStart" class="w-100" format="DD/MM/YYYY"
                    placeholder="Select start date" :disabled-date="disableStartDate" @change="onStartDateChange" />
                  <small v-if="formErrors.payPeriodStart" class="text-danger">{{ formErrors.payPeriodStart }}</small>
                </div>

                <div class="col-md-6">
                  <label class="form-label">Pay Period End Date <span class="text-danger">*</span></label>
                  <a-date-picker v-model:value="formData.payPeriodEnd" class="w-100" format="DD/MM/YYYY"
                    placeholder="Select end date" :disabled-date="disableEndDate" @change="onEndDateChange" />
                  <small v-if="formErrors.payPeriodEnd" class="text-danger">{{ formErrors.payPeriodEnd }}</small>
                </div>

                <div class="col-md-6">
                  <label class="form-label">Payment Date <span class="text-danger">*</span></label>
                  <a-date-picker v-model:value="formData.paymentDate" class="w-100" format="DD/MM/YYYY"
                    placeholder="Select payment date" :disabled-date="disablePaymentDate" />
                  <small v-if="formErrors.paymentDate" class="text-danger">{{ formErrors.paymentDate }}</small>
                </div>

                <div class="col-md-6">
                  <label class="form-label">Payroll Month <span class="text-danger">*</span></label>
                  <a-select v-model:value="formData.payrollMonth" class="w-100" placeholder="Select month">
                    <a-select-option v-for="month in months" :key="month.value" :value="month.value">
                      {{ month.label }}
                    </a-select-option>
                  </a-select>
                  <small v-if="formErrors.payrollMonth" class="text-danger">{{ formErrors.payrollMonth }}</small>
                </div>
              </div>
            ...
            <div v-show="currentStep === 2">
              <div class="mb-3">
                <p class="text-muted mb-3">
                  <i class="ti ti-info-circle me-1"></i>
                  Select employees to include in this payroll batch. Use filters to narrow down your selection.
                </p>
              </div>
              ...
              <div class="row g-2 mb-3">
                ...
                <div class="col-md-3">
                  <a-select v-model:value="employeeFilters.department" class="w-100" placeholder="All Departments"
                    allow-clear @change="handleEmployeeFilterChange">
                ...
                <div class="col-md-2">
                  <a-select v-model:value="employeeFilters.status" class="w-100" placeholder="Status" allow-clear
              ...
              <div class="d-flex justify-content-between align-items-center mb-3 p-2 bg-light rounded">
                ...
                <div>
                  <button type="button" class="btn btn-sm btn-primary me-2" @click="selectAllEmployees"
                    :disabled="filteredEmployees.length === 0">
                    Select All
                  </button>
                  <button type="button" class="btn btn-sm btn-secondary" @click="clearEmployeeSelection"
              ...
              <div v-if="!loadingEmployees" class="employee-list">
                <div v-if="filteredEmployees.length > 0" class="row g-2">
                  <div v-for="employee in paginatedEmployees" :key="employee.id" class="col-md-4">
                    <div class="employee-item" :class="{ 'selected': isEmployeeSelected(employee.id) }"
                      @click="toggleEmployeeSelection(employee.id)">
                      <div class="form-check">
                        <input class="form-check-input" type="checkbox" :checked="isEmployeeSelected(employee.id)"
                          @click.stop="toggleEmployeeSelection(employee.id)" />
                        <label class="form-check-label">
                          <div class="fw-semibold">{{ employee.name }}</div>
                          <small class="text-muted">{{ employee.staff_id }} • {{ employee.department }}</small>
                        </label>
                      </div>
                    </div>
                  </div>
                ...
                <nav v-if="filteredEmployees.length > employeesPerPage" class="mt-3">
                  <ul class="pagination pagination-sm justify-content-center">
                    <li class="page-item" :class="{ disabled: employeeCurrentPage === 1 }">
                      <a class="page-link" href="#" @click.prevent="employeeCurrentPage = Math.max(1, employeeCurrentPage - 1)">Previous</a>
                    </li>
                    <li class="page-item" :class="{ active: employeeCurrentPage === page }"
                      v-for="page in Math.min(employeeTotalPages, 5)" :key="page">
                      <a class="page-link" href="#" @click.prevent="employeeCurrentPage = page">{{ page }}</a>
                    </li>
                    <li class="page-item" :class="{ disabled: employeeCurrentPage === employeeTotalPages }">
                      <a class="page-link" href="#" @click.prevent="employeeCurrentPage = Math.min(employeeTotalPages, employeeCurrentPage + 1)">Next</a>
                    </li>
                  </ul>
                </nav>
              </div>
                ...
                <table class="table table-sm table-hover">
                  ...
                  <tbody>
                    <tr v-for="(employeeId, index) in selectedEmployees" :key="employeeId">
                      ...
                      <td>
                        <button type="button" class="btn btn-sm btn-outline-danger" @click="removeEmployee(employeeId)">
                          <i class="ti ti-x"></i>
                        </button>
                      </td>
                    </tr>

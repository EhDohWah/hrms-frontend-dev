<template>
    ...
    <OverallPayrollReportModal ref="overallPayrollReportModal" :pdf-url="pdfUrl" />
</template>
...
import { reportPayrollService } from '@/services/report-payroll.service';
import OverallPayrollReportModal from '@/components/modal/reports/overall-payroll-report-modal.vue';

export default {
    components: {
        ReportRow,
        OverallPayrollReportModal
    },
    data() {
        return {
            report: {
    ...
    methods: {
        async preview() {
            try {
                ...
                const pdfBlob = await reportPayrollService.generatePayrollReportPDF(startDate, endDate);

                // Create a URL for the blob
                this.pdfUrl = window.URL.createObjectURL(pdfBlob);

                // Open the modal using the reference
                this.$refs.overallPayrollReportModal.openModal();
        ...
        async exportPDF() {
            try {
                ...
                a.href = url;
                a.download = `payroll-report-${startDate}-to-${endDate}.pdf`;
        ...
        async exportExcel() {
            try {
                ...
                const excelBlob = await reportPayrollService.generatePayrollReportExcel(startDate, endDate);
        ...
        exportCSV() {
            message.success('Exported Payroll Report as CSV');
        }
    },
    mounted() {

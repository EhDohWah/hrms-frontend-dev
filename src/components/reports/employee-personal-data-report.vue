<template>
    ...
    <OverallEmployeePersonalDataReportModal ref="overallEmployeePersonalDataReportModal" :pdf-url="pdfUrl" />
</template>
...
import { reportEmployeePersonalDataService } from '@/services/report-employee-personal-data.service';
import OverallEmployeePersonalDataReportModal from '@/components/modal/reports/overall-employee-personal-data-report-modal.vue';

export default {
    components: {
        ReportRow,
        OverallEmployeePersonalDataReportModal
    },
    data() {
        return {
            report: {
    ...
    methods: {
        async preview() {
            try {
                ...
                const pdfBlob = await reportEmployeePersonalDataService.generateEmployeePersonalDataReportPDF(startDate, endDate);

                // Create a URL for the blob
                this.pdfUrl = window.URL.createObjectURL(pdfBlob);

                // Open the modal using the reference
                this.$refs.overallEmployeePersonalDataReportModal.openModal();
        ...
        async exportPDF() {
            try {
                ...
                a.href = url;
                a.download = `employee-personal-data-report-${startDate}-to-${endDate}.pdf`;
        ...
        async exportExcel() {
            try {
                ...
                const excelBlob = await reportEmployeePersonalDataService.generateEmployeePersonalDataReportExcel(startDate, endDate);

                // Create a URL for the blob and trigger download
                const url = window.URL.createObjectURL(excelBlob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `employee-personal-data-report-${startDate}-to-${endDate}.xlsx`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
        ...
        exportCSV() {
            message.success('Exported Employee Personal Data Report as CSV');
        }
    },
    mounted() {

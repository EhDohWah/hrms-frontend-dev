<template>
    ...
    <OverallTotalGrantReportModal ref="overallTotalGrantReportModal" :pdf-url="pdfUrl" />
</template>
...
import { reportTotalGrantService } from '@/services/report-total-grant.service';
import OverallTotalGrantReportModal from '@/components/modal/reports/overall-total-grant-report-modal.vue';

export default {
    components: {
        ReportRow,
        OverallTotalGrantReportModal
    },
    data() {
        return {
            report: {
    ...
    methods: {
        async preview() {
            try {
                ...
                const pdfBlob = await reportTotalGrantService.generateTotalGrantReportPDF(startDate, endDate);

                // Create a URL for the blob
                this.pdfUrl = window.URL.createObjectURL(pdfBlob);

                // Open the modal using the reference
                this.$refs.overallTotalGrantReportModal.openModal();
        ...
        async exportPDF() {
            try {
                ...
                a.href = url;
                a.download = `total-grant-report-${startDate}-to-${endDate}.pdf`;
        ...
        async exportExcel() {
            try {
                ...
                const excelBlob = await reportTotalGrantService.generateTotalGrantReportExcel(startDate, endDate);

                // Create a URL for the blob and trigger download
                const url = window.URL.createObjectURL(excelBlob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `total-grant-report-${startDate}-to-${endDate}.xlsx`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
        ...
        exportCSV() {
            message.success('Exported Total Grant Report as CSV');
        }
    },
    mounted() {

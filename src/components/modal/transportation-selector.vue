    ...
    <div class="transportation-selector">
        <div class="row">
            <div v-for="option in options" :key="option.value" :class="getColumnClass()">
                <div class="form-check mb-2">
                    <input class="form-check-input" type="radio" :name="radioName" :id="`transport_${option.value}`"
                        :value="option.value" :checked="selectedOption === option.value" @change="onOptionChange" />
                    <label class="form-check-label" :for="`transport_${option.value}`">
                        {{ option.label }}
                    </label>
                </div>
            </div>
        ...
        <div v-if="selectedOption === 'other'" class="mt-2">
            <input v-model="customValue" type="text" class="form-control"
                placeholder="Please specify transportation method..." @input="onCustomValueChange" maxlength="200" />
        </div>
    </div>
</template>
...
<style scoped>
.transportation-selector .form-check {
    display: flex;
    align-items: center;
    min-height: 32px;
}

.transportation-selector .form-check-input {
    margin-right: 8px;
}

.transportation-selector .form-check-label {
    margin-bottom: 0;
    cursor: pointer;
    font-size: 14px;
}

.transportation-selector .form-control {
    font-size: 14px;
}
</style>
